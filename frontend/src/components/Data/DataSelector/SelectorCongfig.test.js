...
describe('When the user evaluates the content scraping', () => {
    ...
    test('when the evaluation is successful', async () => {
        ...
        expect(onConfigured.mock.calls[0][0]).toStrictEqual({ "name": "test-data", "selector": { "path": ".a-good-selector", "status": "valid" }, "isPopup": undefined, "popupSelector": undefined });
...
describe('When the popup selector is activated', () => {
    ...
    test('when the switch button is activated', async () => {
        ...
        expect(onConfigured.mock.calls.length).toBe(1);
        expect(onConfigured.mock.calls[0][0]).toStrictEqual({ "name": "test-data", "selector": { "path": ".a-good-selector-with-popup", "status": "valid" }, 'isPopup': true, 'popupSelector': { "path": ".a-popup-selector", "status": "valid" } });
