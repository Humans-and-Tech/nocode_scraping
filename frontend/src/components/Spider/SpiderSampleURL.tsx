...
export const SpiderSampleURL = ({ spider, ...rest }: { spider: Spider }): JSX.Element => {
  ...
  const btnText = () => {
    return (!spider.sampleURLs || spider.sampleURLs?.length==0) ?  t('spider.actions.sample_urls_add_first_one'): t('spider.actions.sample_urls_add_more')
  };
          ...
          </Button>
          {
            ...
            localSpider.sampleURLs && (
              <List
                ...
                renderItem={item => <List.Item actions={[<a onClick={()=> {deleteSampleUrl(item)}} key="list-loadmore-edit">{t('spider.actions.sample_url_delete')}</a>]}>{item.toString()}</List.Item>}
              />
            )
          }
