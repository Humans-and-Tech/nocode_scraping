...
export const SpiderSampleURL = (): JSX.Element => {
  ...
  const changeDisplayMode = (): void =>
    mode == DisplayMode.VIEW ? setMode(DisplayMode.EDIT) : setMode(DisplayMode.VIEW);
    ...
    <>
      {mode == DisplayMode.VIEW ? (
        <Space direction="vertical" size="middle">
          <span data-testid="sample-url-view-mode">
            {t('there are x sample urls')}
            {spider?.sampleURLs?.length}
          </span>
