 ...
 * @returns
 */
export const cachePageContent = async (key: string, content: string): Promise<boolean | undefined> => {
    ...
    brotliCompress(Buffer.from(content), async function (err: Error | null, compressed: Buffer) {
      ...
      try {
        if (compressed) {
          ...
          const b: boolean = await upsert({}, { content: compressed.toString('base64') }, document);
 ...
 */
export const loadPageContentFromCache = async (key: string): Promise<ICachedContent | undefined> => {
  const organizationName = 'test';
  const configCollection: DocumentData = firestore.collection(`organizations`);
  const document = configCollection.doc(`${organizationName}/cache/${key}`);
