...
export const clickElement = async (
  ...
  selector: DataSelector
    ...
    if (selector !== undefined && selector.path !== undefined) {
        ...
        try {
            ...
            if (validityResponse.status === GenericResponseStatus.ERROR) {
                return Promise.reject(
                    new ScrapingError(`Error validating the selector ${selector}`, ScrapingStatus.ERROR, validityResponse.selector)
                );
        ...
        try {
            ...
            if (err instanceof playwright.errors.TimeoutError) {
                return Promise.reject(
                    new ScrapingError(`the selector ${selector.path} could not be found`, ScrapingStatus.ELEMENT_NOT_FOUND, selector)
                );
            } 
            return Promise.reject(
    ...
    return Promise.reject(
        new ScrapingError(`Undefined selector or selector path`, ScrapingStatus.ERROR, selector) 
    )
};
...
const loadPage = async (url: URL): Promise<playwright.Page> => {
    ...
    try {
        ...
        if (cachedHtml !== undefined) {
            console.debug('Using cached content for key', key);
            await page.setContent(cachedHtml);
        } else {
            console.warn('Downloading content from the web for key', key);
...
export const getContent = async (req: IScrapingRequest): Promise<ScrapedContent | ScrapingError> => {
    ...
    if (req === undefined || req.selector === undefined || req.selector.path === undefined) {
        return Promise.reject(
            new ScrapingError('invalid call', ScrapingStatus.ERROR, undefined)
        );
    }
    ...
    try {
        ...
        if (validityResponse.status === GenericResponseStatus.ERROR) {
            return Promise.reject(
                new ScrapingError(`Error validating the selector ${req.selector}`, ScrapingStatus.ERROR, validityResponse.selector)
            );
        } else if (validityResponse?.selector?.status === SelectorStatus.INVALID) {
            return Promise.reject(
                new ScrapingError(`Invalid selector ${req.selector}`, ScrapingStatus.INVALID_SELECTOR, validityResponse.selector)
            );
        }
        ...
        return Promise.reject(
            new ScrapingError(`invalid selector, ${err}`, ScrapingStatus.ERROR, req.selector)
        );
    }
    ...
    try {
        ...
        if (content) {
            return Promise.resolve(
                new ScrapedContent(content, req.selector, `data:image/gif;base64,${imageAsBase64}`)
            );
        } else {
            return Promise.reject(
