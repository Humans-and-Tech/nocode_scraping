...
export class ScrapingService {
   ...
   */
  async validateSelector(selector: DataSelector): Promise<DataSelectorValidityResponse | DataSelectorValidityError> {
   ...
   * @returns
   */
  async clickElement(page: playwright.Page, selector: DataSelector): Promise<ScrapingError | boolean> {
    if (selector !== undefined && selector.path !== undefined) {
      ...
      try {
        const validityResponse = await this.validateSelector(selector);
   ...
   */
  async getContent(req: IScrapingRequest): Promise<ScrapedContent | ScrapingError> {
    ...
    try {
      ...
      if (content) {
        ...
        return Promise.reject(
          new ScrapingError(
            `no content found for selector ${req.selector.path}`,
